    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                self.ant, insect = insect, self.ant
                # BEGIN Problem 9
                self.ant.ant = insect
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if self.place.bees != []:
            return random_or_none(self.place.bees)
        if self.place.bees == []:
            while self.place.entrance.bees == [] and self.place.entrance.name != 'Hive':
        ...
        while my_loc != hive:
            if (self.min_range <= count and self.max_range >= count and my_loc.bees):
                return random_or_none(my_loc.bees)
            else:
                count += 1
                my_loc = my_loc.entrance
        # END Problem 3 and 4
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.watersafe:    # check if watersafe
...
class FireAnt(Ant):
    ...
    food_cost = 5
    implemented = False
    blocks_path = True   # Change to True to view in the GUI
    # END Problem 5
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt performs no action, but has high armor"""
    name = 'Wall'
    implemented = True
    food_cost = 4
    blocks_path = True
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        beeies = list(self.place.bees)
        for bb in beeies:
            bb.reduce_armor(self.damage)
        # END Problem 7
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        # placey = Place('placey', exit = None)
        copy_of_bees = colony.bees[:]
        # print(copy_of_bees)
    ...
    def action(self, colony):
        ...
        if(self.true_queen == False):
            ...
            while temp_place is not None:
                if isinstance(temp_place.ant, Ant) and temp_place.ant not in self.ants_that_are_doubled:
                    temp_place.ant.damage = (temp_place.ant.damage * 2)
                    self.ants_that_are_doubled.append(temp_place.ant)
                if isinstance(temp_place.ant, Ant) and temp_place.ant.container and isinstance(temp_place.ant.ant, Ant) and temp_place.ant.ant not in self.ants_that_are_doubled:
                    temp_place.ant.ant.damage = (temp_place.ant.ant.damage * 2)
                    self.ants_that_are_doubled.append(temp_place.ant.ant)
...
def make_slow(action):
    ...
    # BEGIN Problem EC
    count = 0
    duration = apply_effect.duration
    def return_function(colony):
        nonlocal count
        count += 1
        if colony.time % 2 == 0 or count > duration:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    apply_effect.duration = duration #effect of bee should be the same as duration if >= 0
